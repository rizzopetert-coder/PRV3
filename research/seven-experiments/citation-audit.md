...
- **Forward-looking note only:** whenever Hammer Index citations do get sourced for this state, they should be selected and framed with the current enforcement direction in mind (EEOC risk currently points toward exclusionary/preferential DEI mechanics, not failure-to-promote) — but that's guidance for future citation-building work, not a fix to anything currently in place.
...
5. Source Hammer Index citations for The Diversity Ceiling, which currently appears to have none — minor, separate gap noted in Section 1a, not urgent.
