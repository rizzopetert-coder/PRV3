...
| `silosolation` | 1 — Blends in | SAME-CLUSTER DIFF (confirmed) | rank-6, full 3-way tie w/ `the_second_close`, `the_arbitrary_standard` | DIAGNOSED | Confirmed Alliance-correct — cross-team information isolation, explicitly "not hostile... structural," not about relationship history. Distinct from `the_second_close`'s relational-trust story. | Alliance | Real descriptive_prose: "Teams that need each other's information to do their jobs well are operating as if they don't, each optimizing for its own metrics without visibility into how that affects anyone else." Decomposition (tools/_scdwcs_decomposition_rank6.py) confirms `silosolation` has zero independent false-rank-1 signal of its own — every win it would score is masked behind `the_second_close`'s earlier registration order (byte-identical scores confirmed on both of rank-6's 2 genuine external wins). Its own 3 dedicated profiles are entirely lost to `the_second_close` in the current tie (registration-order artifact, not a real loss). |
| `the_arbitrary_standard` | 1 — Blends in | SAME-CLUSTER DIFF (confirmed) | rank-6, full 3-way tie | DIAGNOSED | Confirmed Alliance-correct — inconsistent, non-transparent rule application producing differential treatment. A fairness/equity-in-treatment story (about how people are treated relative to each other), not a re-clustering candidate despite the Legal & Compliance liability axis. | Alliance | Real descriptive_prose: "The rules that govern who gets what treatment aren't applied consistently, and the pattern of who benefits isn't accidental even if nobody designed it on purpose." SEPARATE CAVEAT, not part of the rank-6 tie question: its own 3 dedicated profiles (ALL-AS-01/02/03) are currently lost outright to Track 2 broad attractors (`built_to_fail`, `invisible_performance_management`) by a REAL score gap (e.g. 0.750 vs. the rank-6 trio's 0.674 on ALL-AS-01), not a tie — confirmed via direct decomposition. A same-cluster rank-6 pilot would not fix this by itself; it's Track 2's already-flagged dominance problem sitting on top of the rank-6 question, same shape as `built_to_fail`'s residual against `the_paper_tiger` after rank-8's pilot. |
