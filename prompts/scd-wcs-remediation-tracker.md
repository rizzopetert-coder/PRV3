...
| `invisible_influence_architecture` | 1 — Blends in | SAME-CLUSTER DIFF (confirmed) | rank-5, full 3-way tie | DIAGNOSED | Confirmed Authority-correct — needs same-cluster differentiation from its 2 cluster-mates, not a re-clustering question. | Authority | Real Authority match confirmed against live descriptive_prose: "the formally accountable people are not always the ones actually deciding outcomes" is a clean decision-rights story. No pushback needed — text supports the existing vector's Authority-dominant read as-is. |
| `planning_authority_gap` | 1 — Blends in | SAME-CLUSTER DIFF (confirmed) | rank-5, full 3-way tie | DIAGNOSED | Confirmed Authority-correct — needs same-cluster differentiation. | Authority | Real Authority match confirmed: "don't hold the authority to make the decisions their plans depend on" is textbook Authority — name and text align. |
