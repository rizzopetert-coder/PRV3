...
## Verification requirement — read before answering

This request carries more structural weight than the last two SCD-WCS reviews sent to you, both of which had real, catchable errors (a stale `invisible_performance_management` baseline value; a proposed vector whose own stated fields didn't sum to the invariant you yourself cited) — caught precisely because those claims were specific and checkable against live code. A purely theoretical answer here, with no pointer back to the actual implementation, will be much harder to verify before Pete decides whether to act on it.

**Any claim you make about the scoring geometry's actual behavior — whether it conserves false-rank-1 mass, whether redistribution vs. reduction is possible, how a specific field or mechanism behaves — must cite the specific function, file, and line it's reasoning from, not just restate or paraphrase the formula already given above.** If a claim can't be tied to a specific place in `engine/accumulation.py` (or another real file, cited by path and line), say so explicitly rather than presenting it as if it were grounded. General mathematical reasoning about cosine similarity in the abstract is fine and expected — just be explicit about which parts are abstract math and which parts are claims about this specific codebase's actual behavior, and cite the latter.
