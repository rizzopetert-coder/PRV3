...
_QDATA = [
    ...
    (
        ...
        [
            ...
            ("B", "Reviewed within the past 1-3 years, but not specifically against current law.", False, None),
            ("C", "Formatted or lightly edited since then, but not substantively reviewed.", False, None),
            ("D", "Not sure when it was last genuinely reviewed.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-HRPOL-02",
        "Do employees actually sign an acknowledgment when the handbook changes, and is that record retrievable if you needed it tomorrow?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, every employee signs, and the record is retrievable on demand.", False, None),
            ("B", "Most employees sign, but tracking is inconsistent.", False, None),
            ("C", "Acknowledgment exists as a formality but isn't reliably collected.", False, None),
            ("D", "No acknowledgment process exists.", False, None),
        ],
        [],
        False,
    ),
    ...
    (
        "TC-HIRE-02",
        "When a background check comes back with something on it, is there a documented process for what happens next, or does someone just make a judgment call?",
        "forced_choice", None, "tactical",
        [
            ("A", "There's a documented adjudication process everyone follows.", False, None),
            ("B", "There's an informal norm, but nothing written down.", False, None),
            ("C", "It's handled case by case with no consistent standard.", False, None),
            ("D", "This hasn't come up, or I'm not sure how it's handled.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-HIRE-03",
        "What does a new hire's first two weeks actually look like, and how much of that exists on paper versus in someone's head?",
        "forced_choice", None, "tactical",
        [
            ("A", "Documented onboarding plan, consistent across hires.", False, None),
            ("B", "A general plan exists but execution varies by manager.", False, None),
            ("C", "Mostly informal, lives in individual managers' heads.", False, None),
            ("D", "No defined onboarding process.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-HIRE-04",
        "Is there a completed I-9 on file for every current employee, and could you produce them all today if asked?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, complete and auditable today.", False, None),
            ("B", "Mostly complete, with a few known gaps.", False, None),
            ("C", "Not confident all are complete.", False, None),
            ("D", "Not sure without checking.", False, None),
        ],
        [],
        False,
    ),
    ...
    (
        "TC-PAY-03",
        "Who reviews payroll before it runs, and what would they actually catch if something were wrong?",
        "forced_choice", None, "tactical",
        [
            ("A", "A defined reviewer checks specific things before every run.", False, None),
            ("B", "Someone reviews it, but the process isn't clearly defined.", False, None),
            ("C", "Payroll runs with minimal review.", False, None),
            ("D", "No one reviews it before it runs.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-PAY-04",
        "Has anyone audited pay stubs against state-specific wage statement requirements, or is the assumption that the payroll provider handles it?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, audited directly against requirements.", False, None),
            ("B", "Assumed the provider handles it, never independently checked.", False, None),
            ("C", "Never considered until now.", False, None),
            ("D", "Not sure what the requirements even are.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-COMPC-01",
        "Do your job postings currently include pay ranges, and do those ranges match what the role has actually been paid?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, and ranges reflect actual pay.", False, None),
            ("B", "Ranges are included but don't always match reality.", False, None),
            ("C", "Ranges are included in some postings, not others.", False, None),
            ("D", "No pay ranges included.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-COMPC-02",
        "Has anyone reviewed pay equity across employees doing substantially similar work, or is the belief that pay is fair based on intent rather than data?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, a formal pay equity review has been done.", False, None),
            ("B", "Informally reviewed, no formal analysis.", False, None),
            ("C", "Never reviewed, but no specific concerns raised.", False, None),
            ("D", "Never reviewed, and I genuinely don't know what we'd find.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-COMPC-03",
        "Are your minimum wage rates current for every location you have employees in, including remote workers in other states?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, tracked and updated by location including remote employees.", False, None),
            ("B", "Tracked for physical locations, less certain about remote.", False, None),
            ("C", "Tracked loosely, not location-specific.", False, None),
            ("D", "Not actively tracked.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-COMPC-04",
        "If asked to produce your compensation philosophy in writing, does one exist, or would it need to be reconstructed from what people remember deciding?",
        "forced_choice", None, "tactical",
        [
    ...
    (
        ...
        [
            ...
            ("B", "Assumed the broker handles it, not independently verified.", False, None),
            ("C", "Not sure what's actually being sent.", False, None),
            ("D", "No tracking exists either way.", False, None),
        ],
        [],
        False,
    ),
    (
        ...
        [
            ...
            ("B", "We'd probably know eventually, proof might be harder.", False, None),
            ("C", "We likely wouldn't know unless they said something.", False, None),
            ("D", "No way to know or prove this currently.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-BEN-03",
        "Is your ACA reporting current, and does whoever owns it actually understand what triggers a filing obligation, or are they just following last year's process?",
        "forced_choice", None, "tactical",
        [
            ("A", "Current, and re-evaluated annually against actual headcount and coverage.", False, None),
            ("B", "Current, but following last year's process without re-evaluation.", False, None),
            ("C", "Uncertain whether it's fully current.", False, None),
            ("D", "Not confident this is being handled correctly.", False, None),
        ],
        [],
        False,
    ),
    ...
    (
        "TC-LEAVE-01",
        "When a leave request comes in, is there a defined process for determining eligibility, or does it get figured out case by case?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, a consistent, defined eligibility process.", False, None),
            ("B", "General practice exists but isn't formally defined.", False, None),
            ("C", "Handled case by case.", False, None),
            ("D", "No consistent process at all.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-LEAVE-02",
        "How is intermittent leave actually tracked day to day, and would that tracking survive a request to reconstruct six months of it?",
        "forced_choice", None, "tactical",
        [
            ("A", "Tracked systematically, reconstructable at any point.", False, None),
            ("B", "Tracked, but with gaps that would complicate reconstruction.", False, None),
            ("C", "Tracked informally, hard to reconstruct.", False, None),
            ("D", "Not confident it's being tracked at all.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-LEAVE-03",
        "Is there a defined interactive process for accommodation requests, or does it depend on who receives the request?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, a consistent process regardless of who receives the request.", False, None),
            ("B", "General understanding exists, not formally documented.", False, None),
            ("C", "Depends heavily on who receives it.", False, None),
            ("D", "No defined process.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-LEAVE-04",
        "When someone returns from leave, is there a documented return-to-work step, or does it just happen informally?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, documented every time.", False, None),
            ("B", "Usually happens, not always documented.", False, None),
            ("C", "Informal, inconsistent.", False, None),
            ("D", "No defined return step.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-SAFE-01",
        "Are OSHA-required logs current, and does whoever's responsible for them actually know the current recordkeeping thresholds?",
        "forced_choice", None, "tactical",
        [
            ("A", "Current, and thresholds are actively monitored as headcount changes.", False, None),
            ("B", "Current, but not actively re-evaluated as things change.", False, None),
            ("C", "Uncertain if fully current.", False, None),
            ("D", "Not confident this is being maintained correctly.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-SAFE-02",
        "If a workplace injury happened tomorrow, is there a clear, known process for what happens in the first hour, or would it be improvised?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, a clear, known first-hour process.", False, None),
            ("B", "General understanding exists, not formally documented.", False, None),
            ("C", "Would likely be improvised.", False, None),
            ("D", "No process exists.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-SAFE-03",
        "Has your safety policy actually been reviewed since it was written, or has it just been carried forward year to year?",
        "forced_choice", None, "tactical",
        [
            ("A", "Reviewed and updated on a defined cadence.", False, None),
            ("B", "Reviewed occasionally, no defined cadence.", False, None),
            ("C", "Carried forward without review.", False, None),
            ("D", "Not sure when it was last reviewed, if ever.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-SAFE-04",
        "Do employees actually know the safety policy exists and what it requires of them, or does it live in a folder nobody opens?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, actively communicated and reinforced.", False, None),
            ("B", "Communicated at onboarding, not reinforced after.", False, None),
            ("C", "Exists, but employees likely aren't aware of it.", False, None),
            ("D", "Not confident employees know it exists.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-REC-01",
        "Do you have a defined retention schedule for personnel files, or does everything just get kept indefinitely because nobody's sure what's safe to remove?",
        "forced_choice", None, "tactical",
        [
    ...
    (
        "TC-REC-02",
        "Who actually has access to personnel files, and is that access limited to people who need it for a specific reason?",
        "forced_choice", None, "tactical",
        [
            ("A", "Access is limited and reviewed periodically.", False, None),
            ("B", "Access is limited, but never formally reviewed.", False, None),
            ("C", "Access is broader than it probably should be.", False, None),
            ("D", "Not sure who currently has access.", False, None),
        ],
        [],
        False,
    ),
    (
        ...
        [
            ...
            ("B", "Yes, though not sure about the specific timeframe requirements.", False, None),
            ("C", "Probably, but it would take some effort to assemble.", False, None),
            ("D", "Not confident we could produce it cleanly.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-REC-04",
        "Are medical and I-9 records stored separately from the general personnel file, as required, or does everything live in one place?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, stored separately as required.", False, None),
            ("B", "Mostly separate, with some exceptions.", False, None),
            ("C", "Not confident they're properly separated.", False, None),
            ("D", "Everything is likely stored together.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-TECH-01",
        "How many separate systems does HR data actually live in right now, and how much of that data has to be manually reconciled between them?",
        "forced_choice", None, "tactical",
        [
            ("A", "One integrated system, minimal manual reconciliation.", False, None),
            ("B", "A few systems, some manual reconciliation required.", False, None),
            ("C", "Several systems, regular manual reconciliation.", False, None),
            ("D", "Not sure how many systems are actually in play.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-TECH-02",
        "If your HRIS and payroll system disagree about an employee's status or pay rate, which one is treated as the source of truth, and does everyone actually know that?",
        "forced_choice", None, "tactical",
        [
            ("A", "Clear source of truth defined and known by everyone involved.", False, None),
            ("B", "A source of truth exists but isn't clearly communicated.", False, None),
            ("C", "No defined source of truth.", False, None),
            ("D", "This has caused confusion before.", False, None),
        ],
        [],
        False,
    ),
    (
        ...
        [
            ...
            ("B", "Some restrictions exist, not comprehensively.", False, None),
            ("C", "Access is broader than intended by default.", False, None),
            ("D", "Not sure what the actual access settings are.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-TECH-04",
        "When an employee leaves, is there a defined process for removing their system access, or does it happen whenever someone remembers to do it?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, access is removed as part of a defined offboarding process.", False, None),
            ("B", "Usually removed, but not on a defined timeline.", False, None),
            ("C", "Removed inconsistently.", False, None),
            ("D", "No defined offboarding process for access.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-PERF-01",
        "Do performance reviews happen on a defined cadence for everyone, or does it depend on the manager?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, consistent cadence across all managers.", False, None),
            ("B", "A cadence exists but isn't consistently followed.", False, None),
            ("C", "Varies significantly by manager.", False, None),
            ("D", "No defined cadence at all.", False, None),
        ],
        [],
        False,
    ),
    (
        "TC-PERF-02",
        "If an underperforming employee were terminated tomorrow, does the documentation trail support that decision, or would it need to be built after the fact?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, documentation exists alongside the ongoing conversations.", False, None),
            ("B", "Some documentation exists, but it's thin.", False, None),
            ("C", "Documentation would mostly need to be built after the fact.", False, None),
            ("D", "Little to no documentation exists currently.", False, None),
        ],
        [],
        False,
    ),
    ...
    (
        "TC-PERF-04",
        "Is there a standard process for performance improvement plans, or does each one get built from scratch depending on who's managing it?",
        "forced_choice", None, "tactical",
        [
            ("A", "Yes, a standard, consistently used process.", False, None),
            ("B", "A general template exists, but use varies.", False, None),
            ("C", "Built from scratch each time.", False, None),
            ("D", "No standard process exists.", False, None),
        ],
        [],
        False,
    ),
