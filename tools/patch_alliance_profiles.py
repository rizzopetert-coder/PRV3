#!/usr/bin/env python
"""
PRV3 — patch_alliance_profiles.py
Creates engine/test_profiles_alliance.py with 18 Alliance Phase 1 test profiles.
...
Session locked design decisions:
  - Silosolation sev_min=Entrenched: weak profile produces Entrenched (no Emerging tier)
  - The Suppression Filter sev_min=Entrenched: same
  - The Second Close: single_state at all signal levels (M&A event self-identifies)
  - ALL-SF-01: post-acquisition context — significant_events = acquisition_or_merger
  - ALL-FR-03, ALL-SI-03, ALL-SF-03, ALL-AS-03, ALL-DB-03: pass_criterion = "top_3"

Usage:
  python tools/patch_alliance_profiles.py --dry-run
  python tools/patch_alliance_profiles.py --write
"""
import argparse
import sys
from pathlib import Path

TARGET = Path(__file__).parents[1] / "engine" / "test_profiles_alliance.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create engine/test_profiles_alliance.py"
    )
    ...
    if args.dry_run:
        ...
        print(f"  Profiles in CONTENT: {sum(1 for l in lines if 'profile_type=' in l)}")
        print(f"  Unique target_states: {len(set(l for l in lines if 'target_state=' in l))}")
        print()
        rows = []
        cur_id = cur_type = cur_state = None
        for line in lines:
            line = line.strip()
            if line.startswith('test_id='):
                ...
                if cur_id and cur_type and cur_state:
                    rows.append((cur_id, cur_state, cur_type))
                    cur_id = cur_type = cur_state = None
        print(f"  {'test_id':<14} {'target_state':<32} {'profile_type'}")
        print(f"  {'-'*14} {'-'*32} {'-'*20}")
        for tid, state, ptype in rows:
            print(f"  {tid:<14} {state:<32} {ptype}")
    ...
    if args.write:
        ...
        print(f"  {sum(1 for l in lines if 'profile_type=' in l)} profiles written")
