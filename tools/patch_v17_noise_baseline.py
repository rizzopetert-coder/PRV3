"""
Patch: engine/output.py — update _PRECOMPUTED_NOISE_BASELINE to v17 values.

v17: signal amplification + neutral drain (Session 23, 2026-05-23).
Monte Carlo N=1000, seed=42, Q01-Q39.

Usage:
  python tools/patch_v17_noise_baseline.py --dry-run
  python tools/patch_v17_noise_baseline.py --write
"""
...
def run(dry_run: bool):
    ...
    print(f"{'=' * 72}")
    print(f"patch_v17_noise_baseline.py — {mode}")
    ...
    if dry_run:
        print("[DRY-RUN] Would apply: _PRECOMPUTED_NOISE_BASELINE v16 -> v17 (47 states)")
        print("[DRY-RUN COMPLETE] 1 change validated. No file written.")
    else:
        new_text = text.replace(OLD, NEW, 1)
        TARGET.write_text(new_text, encoding="utf-8")
        print("[APPLIED] _PRECOMPUTED_NOISE_BASELINE v16 -> v17 (47 states)")
