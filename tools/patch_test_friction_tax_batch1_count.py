...
1 of the PARTIAL-state verification workstream (CT/DE/DC/ME/MD flipped
to CONFIRMED, engine/friction_tax.py) -- 12 -> 17, list extended.

No other fixture in this file references CT/DE/DC/ME/MD (confirmed by
grep before writing the engine-side patch), so this is the only test
change batch 1 needs.

Usage:
    python tools/patch_test_friction_tax_batch1_count.py --dry-run
    python tools/patch_test_friction_tax_batch1_count.py --write
"""
import argparse
import pathlib
import sys
