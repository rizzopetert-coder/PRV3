#!/usr/bin/env python
"""
PRV3 — patch_authority_b2_profiles.py
Creates engine/test_profiles_authority_b2.py with 24 Authority Phase 1 test profiles.
...
Session locked design decisions:
  - AUT-UN-01: high_confidence but severity = Emerging (acute by definition, locked)
  - AUT-PE moderate/weak and AUT-PF moderate/weak: same multi_state pair — intentional
  - AUT-DN-03 co-signal: culture_drift (Attitude cross-dimension)
  - AUT-DN-02 co-signal: leadership_deafness (Attitude cross-dimension)
  - AUT-DP-03: pass_criterion = "top_3" (doc specifies top-2)
  - AUT-DN-03: pass_criterion = "top_3" (doc specifies top-2)
  - AUT-PE-03: pass_criterion = "top_3" (doc specifies top-2)
  - AUT-PF-03: pass_criterion = "top_3" (doc specifies top-2)

Usage:
  python tools/patch_authority_b2_profiles.py --dry-run
  python tools/patch_authority_b2_profiles.py --write
"""
import argparse
import sys
from pathlib import Path

TARGET = Path(__file__).parents[1] / "engine" / "test_profiles_authority_b2.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create engine/test_profiles_authority_b2.py"
    )
    parser.add_argument(
        ...
        for line in lines:
            ...
            if line.startswith('test_id='):
                ...
                if current_id and current_type and current_state:
                    ...
                    current_id = current_type = current_state = None
        print(f"  {'test_id':<14} {'target_state':<38} {'profile_type'}")
        print(f"  {'-'*14} {'-'*38} {'-'*20}")
        for tid, state, ptype in rows:
            print(f"  {tid:<14} {state:<38} {ptype}")
