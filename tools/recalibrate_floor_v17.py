"""
PRV3 Signal Floor Recalibration — v17 (Session 23)
...
Run after v17 signal amplification + neutral drain writes.

Usage:
  python tools/recalibrate_floor_v17.py
"""
...
# v16 baseline for delta reporting
V16_BASELINE = {
    "built_to_fail":                        0.8274,
    "culture_drift":                        0.9198,
    "decision_blindness":                   0.7363,
    "decision_paralysis":                   0.9439,
    "dueling_narratives":                   0.9439,
    "groundhog_day":                        0.8988,
    "heard_and_ignored":                    0.9147,
    "hr_capture":                           0.9147,
    "identity_erosion":                     0.8936,
    "invisible_burnout":                    0.8988,
    "invisible_influence_architecture":     0.9020,
    "leadership_continuity_risk":           0.9439,
    "leadership_deafness":                  0.8936,
    "narrative_lock":                       0.8936,
    "paper_shield":                         0.9020,
    "pay_exposure":                         0.9439,
    "silosolation":                         0.7840,
...
if __name__ == "__main__":
    ...
    shifted = {
        sid: (baseline[sid], V16_BASELINE.get(sid, 0.0),
              baseline[sid] - V16_BASELINE.get(sid, 0.0))
        for sid in baseline
        if abs(baseline[sid] - V16_BASELINE.get(sid, 0.0)) > 0.005
    }

    print(f"\nSignal Floor Recalibration — v17 (weighted cosine, Session 23)")
    print(f"=" * 64)
    ...
    print(f"\nAggregate results:")
    print(f"  v16 mean baseline: {sum(V16_BASELINE.values()) / len(V16_BASELINE):.4f}")
    print(f"  v17 mean baseline: {mean_baseline:.4f}")
    print(f"  v17 range:         {min_val:.4f} - {max_val:.4f}")
    print(f"\nthe_uninitiated: v16={V16_BASELINE.get('the_uninitiated', 0):.4f}  "
          f"v17={baseline.get('the_uninitiated', 0):.4f}  "
          f"delta={baseline.get('the_uninitiated', 0) - V16_BASELINE.get('the_uninitiated', 0):+.4f}")
    print(f"\nStates shifted >0.005 from v16 ({len(shifted)} states):")
    if shifted:
        for sid in sorted(shifted.keys()):
            v17, v16, delta = shifted[sid]
            sign = "+" if delta >= 0 else ""
            print(f"  {sid:<45} v16={v16:.4f}  v17={v17:.4f}  delta={sign}{delta:.4f}")
    else:
        print("  None")

    print(f"\nFull per-state v17 baseline (for engine/output.py update):")
