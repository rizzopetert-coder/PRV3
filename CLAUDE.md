...
### Quarterly Step-Back
A full project assessment (workstream status, goal progress, process feedback) should be run on a calendar cadence: every 2 weeks (biweekly), amended from the original every-3-weeks cadence per Pete's explicit correction, 2026-08-28. Originally defined as "roughly every 15 sessions" (locked at Session 71) — changed to a 3-week calendar cadence because the session-number counter this depended on was discontinued after Session 72 (Section 16 switched to date-based headers with no session numbers), making the original trigger uncheckable; that 3-week cadence itself was then tightened to biweekly. This calendar-based cadence replaces the session-count trigger entirely going forward, not just for this one instance — future step-backs are checked against calendar time from the last logged date below, not a session counter.
...
- Any change that adds, modifies, or depends on API route registrations (e.g. `vercel.json`'s `routes` array, new FastAPI endpoints in `api/engine.py`) requires a live production round-trip check before being marked done. Local/mocked test suites (vitest, tsc, Python unit tests) do not exercise Vercel's actual URL-to-function mapping and cannot catch routing-layer gaps. Locked 2026-08-27 after two same-session confirmations that local-clean did not mean live-clean: the `organization_size` legacy-string bug, and this session's `vercel.json` route-registration gap (Narrative Modulation shipped without it, causing a full production outage: no live Path 1 session could complete, until caught by an unscheduled live verification pass). Third mandatory check alongside the existing Demographic Applicability Filter and dollar-plausibility check.
